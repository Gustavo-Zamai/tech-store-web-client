      ...
      tbody.innerHTML = funcs.value.map(f => `
        <tr>
          ...
          <td><div style="display:flex;gap:.5rem">
            <button class="btn btn-secondary btn-sm" onclick="window.editFuncionario(${f.id})">✏️ Editar</button>
            <button class="btn btn-danger btn-sm" onclick="window.deleteFuncionario(${f.id},'${f.nome}')">🗑️</button>
          </div></td>
        </tr>
      `).join('');
    } else {
      tbody.innerHTML = emptyRow(8, funcs.status === 'rejected' ? 'Erro ao carregar funcionários.' : 'Nenhum funcionário cadastrado.');
    }

    document.getElementById('func-search').addEventListener('input', e => filterTable('table-funcionarios', e.target.value));
    document.getElementById('btn-new-func').addEventListener('click', () => this.openModal(null, empresasList));

    // Expor para uso global (onclick)
    window.editFuncionario = async (id) => {
      const f = await API.funcionarios.get(id);
      this.openModal(f, empresasList);
    };
  ...
  openModal(func, empresas) {
        ...
        <div class="modal" style="max-width:560px">
          ...
          <form id="form-funcionario">
            ...
            <div class="grid-2">
              ...
              <div class="form-group">
                <label>Salário</label>
                <input class="form-control" name="salario" type="number" step="0.01" min="0" value="${func?.salario ?? ''}">
              </div>
            </div>
            <div class="grid-2">
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input class="form-control" name="telefone" value="${func?.telefone ?? ''}">
              </div>
            </div>
            <div class="form-group">
              <label>Empresa</label>
            ...
            <div class="form-group">
              ...
              <input class="form-control" name="endereco" value="${func?.endereco ?? ''}">
            </div>
            <div class="form-group">
    ...
    document.getElementById('form-funcionario').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (isEdit) {
          await API.funcionarios.update(func.id, data);
        } else {
