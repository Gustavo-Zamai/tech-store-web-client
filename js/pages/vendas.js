...
export const VendasPage = {
  async render() {
    ...
    view.innerHTML = `
      <div class="section-header">
        ...
        <div style="display:flex;gap:.75rem;align-items:center">
          <div class="search-bar">
            ...
            <input class="form-control" id="venda-search" placeholder="Buscar venda..." style="width:220px">
          </div>
      ...
      <div class="table-wrapper">
        <table id="table-vendas">
          <thead><tr><th>#</th><th>Cliente</th><th>Funcionário</th><th>Pagamento</th><th>Itens</th><th>Total</th><th>Data</th><th>Situação</th><th>Ações</th></tr></thead>
    ...
    try {
      ...
      tbody.innerHTML = data.map(v => {
        ...
        return `<tr>
          ...
          <td>
            <button class="btn btn-secondary btn-sm" onclick="window.editVenda(${v.id})">✏️ Editar</button>
            <button class="btn btn-danger btn-sm" onclick="window.deleteVenda(${v.id})">🗑️</button>
          </td>
        </tr>`;
      }).join('');
    } catch (e) {
    ...
    document.getElementById('venda-search').addEventListener('input', e => filterTable('table-vendas', e.target.value));
    document.getElementById('btn-new-venda').addEventListener('click', () => this.openModal(null));
  ...
  async openModal(venda) {
            ...
            <div class="grid-2">
              <div class="form-group">
                ...
                <select class="form-control" name="idCliente" required>
                  ...
                  ${clientesList.map(c => `<option value="${c.id}" ${venda?.idCliente == c.id ? 'selected' : ''}>${c.nomeCompleto}</option>`).join('')}
                </select>
              </div>
              <div class="form-group">
            ...
            <div style="margin-bottom:1rem">
              ...
              <div style="display:flex;gap:.5rem;margin-top:.5rem;margin-bottom:.75rem">
                <select class="form-control" id="select-produto" style="flex:1">
                  ...
                  ${prodList.map(p => `<option value="${p.id}" data-preco="${p.precoVenda ?? p.preco ?? p.precoUnitario}" data-nome="${p.nome}">${p.nome} (${formatCurrency(p.precoVenda ?? p.preco ?? p.precoUnitario)})</option>`).join('')}
                </select>
    ...
    const renderItens = () => {
      ...
      container.innerHTML = itens.map((item, idx) => {
        ...
        const preco = prod?.precoVenda ?? prod?.preco ?? prod?.precoUnitario ?? 0;
