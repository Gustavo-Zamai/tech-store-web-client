...
async function pageFornecedores() {
  ...
  document.getElementById('btn-new-forn').addEventListener('click', () => openFornecedorModal(null));

  window.editFornecedor = async (id) => { const f = await API.fornecedores.get(id); openFornecedorModal(f); };
  window.deleteFornecedor = (id, nome) => confirmDelete(nome, async () => {
    try { await API.fornecedores.delete(id); Toast.success('Fornecedor excluído!'); pageFornecedores(); }
