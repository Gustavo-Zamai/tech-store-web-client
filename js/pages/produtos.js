...
export const ProdutosPage = {
  async render() {
    ...
    view.innerHTML = `
      ...
      <div class="table-wrapper">
        <table id="table-produtos">
          <thead><tr><th>#</th><th>Nome</th><th>Categoria</th><th>Fornecedor</th><th>Preço</th><th>Estoque</th><th>Situação</th><th>Ações</th></tr></thead>
    ...
    if (prods.status === 'fulfilled' && prods.value?.length) {
      tbody.innerHTML = prods.value.map(p => {
        const stock = p.quantidadeEstoque ?? p.estoque ?? 0;
        const badge = stock === 0 ? 'badge-danger' : stock < 5 ? 'badge-warning' : 'badge-success';
        const situacaoBadge = p.ativo ? 'badge-success' : 'badge-danger';
        const situacaoText = p.ativo ? 'Ativo' : 'Inativo';
        ...
        return `<tr>
          ...
          <td>${p.descricaoCategoria ?? p.categoria?.nome ?? '—'}</td>
          <td>${p.nomeFornecedor ?? p.fornecedor?.nome ?? '—'}</td>
          <td style="font-weight:600">${formatCurrency(p.precoVenda)}</td>
          <td><span class="badge ${badge}">${stock} un.</span></td>
          <td><span class="badge ${situacaoBadge}">${situacaoText}</span></td>
          <td><div style="display:flex;gap:.5rem">
            <button class="btn btn-secondary btn-sm" onclick="window.editProduto(${p.id})">✏️ Editar</button>
            <button class="btn btn-danger btn-sm" onclick="window.deleteProduto(${p.id},'${p.nome}')">🗑️</button>
          </div></td>
        </tr>`;
      }).join('');
    } else {
      tbody.innerHTML = emptyRow(8, prods.status === 'rejected' ? 'Erro ao carregar produtos.' : 'Nenhum produto.');
    }

    document.getElementById('prod-search').addEventListener('input', (e) => {
      filterTable('table-produtos', e.target.value);
    });

    document.getElementById('btn-new-produto').addEventListener('click', () => 
      this.openModal(null, categoriasList, fornecedoresList)
    );

    // Expor globais
    window.editProduto = async (id) => {
      const p = await API.produtos.get(id);
      this.openModal(p, categoriasList, fornecedoresList);
    };
  ...
  openModal(produto, categorias, fornecedores) {
        ...
        <div class="modal" style="max-width:560px">
          ...
          <form id="form-produto">
            <div class="grid-2">
              ...
              <div class="form-group">
                <label>Preço *</label>
                <input class="form-control" name="precoVenda" type="number" step="0.01" min="0" required value="${produto?.precoVenda ?? ''}">
              </div>
            </div>
            <div class="grid-2">
              <div class="form-group">
                <label>Estoque *</label>
                <input class="form-control" name="quantidadeEstoque" type="number" min="0" required value="${produto?.quantidadeEstoque ?? produto?.estoque ?? 0}">
              </div>
              <div class="form-group">
                <label>Categoria</label>
                <select class="form-control" name="idCategoria">
                  <option value="">— Selecione —</option>
                  ${categorias.map(c => `<option value="${c.id}" ${(produto?.idCategoria ?? produto?.categoria?.id) == c.id ? 'selected' : ''}>${c.nome ?? c.descricaoCategoria}</option>`).join('')}
                </select>
              </div>
            </div>
            <div class="form-group">
    ...
    document.getElementById('form-produto').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (isEdit) {
          await API.produtos.update(produto.id, data);
        } else {
